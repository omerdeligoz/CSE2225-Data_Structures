...
int createLists(FILE *inputFilePtr, node **multiplicandPtr, node **multiplierPtr, node **resultPtr, int *base) {
    ...
    while (((inputChar = fgetc(inputFilePtr)) != EOF) && (inputChar != '\n')) {
        ...
        //Add a node to the linked list using the character read from the file
        *multiplicandPtr = addNode(atoi(&inputChar), *multiplicandPtr);
        count++;
    }
...
//print given linked lists to an output file
void printToFile(node *multiplicandPtr, node *multiplierPtr, node *resultPtr) {
    FILE *outputFilePtr;
...
int main(int argc, char *argv[]) {
    ...
    if (argc >= 2) {
        ...
        printToFile(multiplicandPtr, multiplierPtr, resultPtr);
        printToFile(decimalMultiplicandPtr, decimalMultiplierPtr, decimalResultPtr);
    }
    return 0;
}
